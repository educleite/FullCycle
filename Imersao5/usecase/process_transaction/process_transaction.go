...
import (
	...
	"github.com/educleite/imersao5-gateway/adapter/broker"
	"github.com/educleite/imersao5-gateway/domain/entity"
	"github.com/educleite/imersao5-gateway/domain/repository"
)
...
func (p *ProcessTransaction) Execute(input TransactionDtoInput) (TransactionDtoOutput, error) {
	...
	fmt.Println(input.ID, input.CreditCardName, input.CreditCardNumber)
	...
	if invalidCC != nil {
		return p.rejectTransation(transaction, invalidCC)
	}

	transaction.SetCreditCard(*cc)
	invalidTransaction := transaction.IsValid()
	if invalidTransaction != nil {
		return p.rejectTransation(transaction, invalidTransaction)
	}
...
func (p *ProcessTransaction) rejectTransation(transaction *entity.Transaction, invalidTransaction error) (TransactionDtoOutput, error) {
